...
export const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  return (
    <div className={styles.wrapper} ref={containerRef} style={{ width, maxWidth: '100%' }}>
      ...
      >
        <span className={(multiple && Array.isArray(value) && value.length > 0) || selectedOption ? styles.selectedValue : styles.placeholder} style={size === 'xs' ? { fontSize: '0.73rem', lineHeight: 1.2 } : undefined}>
          {renderTriggerContent()}
        </span>
        <ChevronDown size={size === 'xs' ? 11 : size === 'sm' ? 14 : 16} className={`${styles.icon} ${isOpen ? styles.iconOpen : ''}`} />
      </div>
