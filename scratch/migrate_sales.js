...
async function main() {
  ...
  for (const s of salesToCreate) {
    ...
    const hash = bcrypt.hashSync(s.password, salt).replace(/^\$2a\$/, '$2y$');
    s.hash = hash;
  ...
  const verifyUsers = await query("SELECT id, tenant_id, username, email, full_name, role, status, is_active, is_confirmed, team_id, password_hash FROM users");
