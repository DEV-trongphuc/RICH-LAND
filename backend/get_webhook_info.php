<?php
header('Content-Type: application/json; charset=utf-8');
require_once __DIR__ . '/db_connect.php';

$stmtToken = $conn->query("SELECT setting_value FROM system_settings WHERE setting_key = 'telegram_bot_token' LIMIT 1");
$botToken = $stmtToken->fetch_assoc()['setting_value'] ?? '';

if (empty($botToken)) {
    echo json_encode(["error" => "Bot token is empty"]);
    exit;
}

$url = "https://api.telegram.org/bot{$botToken}/getWebhookInfo";
$ch = curl_init($url);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
$response = curl_exec($ch);
$httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);

$urlMe = "https://api.telegram.org/bot{$botToken}/getMe";
$chMe = curl_init($urlMe);
curl_setopt($chMe, CURLOPT_RETURNTRANSFER, true);
curl_setopt($chMe, CURLOPT_SSL_VERIFYPEER, false);
$responseMe = curl_exec($chMe);
curl_close($chMe);

echo json_encode([
    "bot_token_configured" => !empty($botToken),
    "getMe" => json_decode($responseMe, true),
    "webhookInfo" => json_decode($response, true),
    "http_code" => $httpCode
], JSON_PRETTY_PRINT);
