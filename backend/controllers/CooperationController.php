...
class CooperationController {
    ...
    public function getComments(array $auth, int $id): void {
        $stmt = $this->db->prepare("
            ...
            WHERE c.entity_type = 'cooperation_slip' AND c.entity_id = ? AND c.tenant_id = ?
            ...
            foreach ($mentions as $mUid => $mUser) {
                try {
                    NotificationService::send($this->db, $auth['tenant_id'], 'MENTION_TAGGED', [
                        ...
                        'comment' => "Đã nhắc đến bạn trong thảo luận phiếu hợp tác",
                        'link' => "/cooperation-slips?id=" . $id . "&highlight_comment_id=" . $newId
                    ]);
                } catch (Exception $e) {
                    error_log("Failed to send mention notification for cooperation slip comment: " . $e->getMessage());
                }
            }
        }
